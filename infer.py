...
from paddleseg.utils import logger  # 日志以屏蔽
...
def parse_args():
    ...
    # ___test_修改_one___
    # 配置文件，required=True是强制输入，default无法正常使用，调整required=False
    parser.add_argument(
        "--config",
        dest="cfg",
        help="The config file.",
        default="D:/Code/python/ycProjet/PaddleSeg-2.6.0/PaddleSeg-2.6.0_test/output/infer_model/deploy.yaml",
        type=str,
        required=False)  # default=None , required=True
    # img文件地址或目录地址，支持输入单个文件以及整个文件夹
    parser.add_argument(
        '--image_path',
        dest='image_path',
        help='The directory or path or file list of the images to be predicted.',
        type=str,
        default="D:/Code/python/ycProjet/PaddleSeg-2.6.0/PaddleSeg-2.6.0_test/testimg",
        required=False)  # default=None , required=True
    # ___test_修改_end___
    parser.add_argument(
        '--batch_size',
...
class Predictor:
    ...
    def _init_base_config(self):
        self.pred_cfg = PredictConfig(self.cfg.model, self.cfg.params)  # yaml文件中配置
    ...
    def _init_gpu_config(self):
        ...
        """
        # logger.info("Use GPU")  # ___test_remove___
    ...
    def run(self, imgs_path):
        ...
        # ___test_change_start___
        dataTest = []
        # ___test_change_end___

        for i in range(0, len(imgs_path), args.batch_size):
            # warm up   args.benchmark == False
            ...
            results = self._postprocess(results)  # ___test_change_will___
